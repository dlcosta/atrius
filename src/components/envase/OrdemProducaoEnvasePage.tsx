...
export function OrdemProducaoEnvasePage({ maquinas, produtos }: Props) {
  ...
  useEffect(() => {
    async function carregarOrigens() {
      ...
      try {
        const response = await fetch('/api/novo-fluxo/tanques/origens')
  ...
  useEffect(() => {
    async function carregarAgendaDoDia() {
      ...
      try {
        const response = await fetch(`/api/novo-fluxo/envase?inicio=${dataProducao}&fim=${dataProducao}`)
        const data = await response.json()
  ...
  async function refreshOriginsAndAgenda() {
    try {
      const [origensRes, agendaRes] = await Promise.all([
        fetch('/api/novo-fluxo/tanques/origens'),
        fetch(`/api/novo-fluxo/envase?inicio=${dataProducao}&fim=${dataProducao}`),
      ])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSalvando(true)
    try {
      const response = await fetch('/api/novo-fluxo/envase', {
        method: 'POST',
