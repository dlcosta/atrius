...
export default function CalendarioPage() {
  ...
  const carregarBacklog = useCallback(async () => {
    ...
    try {
      const data = await fetch(apiUrl('/api/backlog')).then((r) => r.json())
  ...
  const carregarBacklogEnvase = useCallback(async () => {
    ...
    try {
      const data = await fetch(apiUrl('/api/backlog/envase')).then((r) => r.json())
  ...
  async function salvarConfiguracaoPedido(ordem: Ordem, setupMin: number, producaoMin: number, limpezaMin: number) {
    ...
    const produtoRes = await fetch(apiUrl('/api/produtos'), {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  async function agendarTanque(ordemId: string, tankId: string, inicio: Date) {
    ...
    const agendRes = await fetch(apiUrl('/api/producao/agendamentos'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ordem_id: ordemId,
